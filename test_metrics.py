...
def test_click_through_rate_returns_a_percentage() -> None:
    if click_through_rate(125, 2_500) != pytest.approx(5.0):
        raise AssertionError("Expected 5.0, but got a different value.")
...
def test_count_campaign_tags_returns_the_number_of_tags() -> None:
    if count_campaign_tags(["video", "search", "priority"]) != 3:
        raise AssertionError("Expected 3 tags, but got a different number.")
